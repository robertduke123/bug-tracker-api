...
app.put('/edit_team', (req, res) => {
    const {oldEmail, newFirst, newLast, newPhone, newEmail, newPosition} = req.body
...
app.delete('/delete_team', (req, res) => {
    ...
    .then(function() {
        ...
        .then(function() {
            db('users').select('*').orderBy('id').then(data => res.json(data))
        })
    })    
})
...
app.get('/team', (req, res) => {
    db('users').select('*').orderBy('id')
    .then(team => res.json(team))
})

app.get('/projects', (req, res) => {
    let finalProjects = []
    db('projects').select('*').orderBy('id')
    .then(projects => {
...
app.put('/edit_project', (req, res) => {
    ...
    db('tickets').where('project_name', '=', project)
    .update({
        project_name: newName
    })
    .returning('*')
    .then(data => {
         db('projects').where('name', '=', project)
        .update({
...
app.delete('/delete_project', (req, res) => {
    ...
    db('projects').where({name: projectName}).del()
    .then(function() {
        db('tickets').where({project_name: projectName}).del()
        .then(function() {
            db('projects').select('*').orderBy('id').then(data => res.json(data))
        })
    })    
})

app.put('/tickets', (req, res) => {
    const {projectName, ticketTitle, author, description, status, priority, type, time, assignedDevs} = req.body
...
app.put('/edit_ticket', (req, res) => {
    const { ticket, newTicketTitle, newAuthor, newDescription, newStatus, newPriority, newType, newTime, newAssignedDevs} = req.body
    db('tickets').where('ticket_title', '=', ticket)
    .update({
...
app.delete('/delete_ticket', (req, res) => {
    ...
    db('tickets').where({ticket_title: ticketName}).del()
    .then(function() {
        db('tickets').select('*').orderBy('id').then(data => res.json(data))
    })    
})
...
app.put('/delete_comment', (req, res) => {
    ...
    db('tickets').select('comment_user', 'comment_date', 'comment_text').where({ticket_title: ticketName})
    .then(data => {
        let user = []
        let date = []
        let text = []
        ...
        db('tickets').where({ticket_title: ticketName})
        .update({
            comment_user: user,
            comment_date: date,
            comment_text: text
        ...
        .then(function() {
            db('tickets').select('*').then(data => res.json(data))
        })
    })  
})
