...
const getTeam = async () => {
	const teamSnapshot = await db.collection("users").get();
	const data = teamSnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
	return data;
};
...
const deleteField = async (collectionName, docId, field) => {
	await db
		.collection(collectionName)
		.doc(docId)
		.update({
...
const editProject = async (data) => {
	const { project, newName, newDescription, newContributors } = data;
	await updateData("tickets", "porject_name", project, {
		project_name: newName,
	});
	await updateData("projects", "name", project, {
		name: newName,
		description: newDescription,
		contributors: newContributors,
	});
...
const deleteProject = async (name) => {
	...
	if (!projectSnapshot.empty) {
		await projectSnapshot.docs[0].ref.delete();
	}
...
const editTicket = async (data) => {
	...
	await updateData("tickets", "ticket_title", ticket, {
		ticket_title: newTicketTitle,
		author: newAuthor,
		description: newDescription,
		status: newStatus,
